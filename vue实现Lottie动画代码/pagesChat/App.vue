  ...
  methods: {
    ...
    insertPrivateMessage(friend, msg) {
      ...
      if (msgType.isRtcPrivate(msg.type)) {
        ...
        let delayTime = 100;
        if (
          msg.type == enums.MESSAGE_TYPE.RTC_CALL_VOICE ||
          msg.type == enums.MESSAGE_TYPE.RTC_CALL_VIDEO
        ) {
          let mode =
            msg.type == enums.MESSAGE_TYPE.RTC_CALL_VIDEO ? "video" : "voice";
      ...
      // 插入消息
      if (
        msgType.isNormal(msg.type) ||
        msgType.isTip(msg.type) ||
        msgType.isAction(msg.type)
      ) {
        let chatInfo = {
          type: "PRIVATE",
          targetId: friend.id,
...
.tab-page {
  ...
  // #ifdef H5
  height: calc(
    100vh - 50px - $im-nav-bar-height
  ); // h5平台100vh是包含了底部高度，需要减去
  top: $im-nav-bar-height;
  // #endif

  // #ifndef H5
  height: calc(
    100vh - var(--status-bar-height) - $im-nav-bar-height
  ); // app平台还要减去顶部手机状态栏高度
