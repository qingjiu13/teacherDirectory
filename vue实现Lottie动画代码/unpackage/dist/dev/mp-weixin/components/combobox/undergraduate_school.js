...
const UndergraduateStore = {
  ...
  mutations: {
    ...
    initFuse(state) {
      ...
      state.schoolFuse = new common_vendor.Fuse(
        schoolItems,
        {
          keys: [
      ...
      common_vendor.index.__f__("log", "at components/combobox/undergraduate_school.js:46", "学校搜索引擎初始化完成，总数量:", schoolItems.length);
    }
  },
  getters: {
    filteredSchools(state) {
      if (!state.filterKeyword) {
        return state.schools.slice(0, 50);
      }
      if (!state.schoolFuse) {
      ...
      if (results.length > 0) {
        common_vendor.index.__f__("log", "at components/combobox/undergraduate_school.js:66", "搜索关键词:", keyword, "找到结果数:", results.length);
      }
      return results.map((result) => result.item.name);
    },
    // 获取最佳匹配的学校（用于自动完成）
    bestMatchedSchool(state, getters) {
      if (!state.filterKeyword || !state.schoolFuse)
        return "";
...
exports.UndergraduateStore = UndergraduateStore;
exports.schoolList = schoolList;
//# sourceMappingURL=../../../.sourcemap/mp-weixin/components/combobox/undergraduate_school.js.map
